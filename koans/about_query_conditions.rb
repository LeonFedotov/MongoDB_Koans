...
class AboutQueryConditions < EdgeCase::Koan
  ...
  def test_select_not_equal_not_not_equal
    assert_equal 99, @numbers.find({'num' => {'$not' => {'$in' => [90]}}}).count, "Select not equal is wrong"
  ...
  def test_select_size
    ...
    assert_equal 0, @arrays.find({'value' => {'$size' => 33}}).count, "Select size 8 is wrong"
  end
  ...
  def test_select_with_regexp
    assert_equal 0, @numbers.find({'string' => /^x/}).count, "Select begins with 1 is wrong"
    assert_equal 0, @numbers.find({'string' => /y$/}).count, "Select ends with 3 is wrong"
  end
